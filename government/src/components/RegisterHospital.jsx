...
function RegisterHospital() {
  const form = useForm({
    ...
    validate: {
      ...
      contactNumber: (value) =>
        /^[0-9]{10}$/.test(value) ? null : 'Contact number must be a 10-digit number',
    },
  });
  ...
  const handleError = (errors) => {
    if (errors.name) {
      ...
      notifications.show({ message: 'Please fill the City field', color: 'red' });
    } else if (errors.contactNumber) {
      notifications.show({ message: 'Please provide a valid Contact Number', color: 'red' });
    }
  };
